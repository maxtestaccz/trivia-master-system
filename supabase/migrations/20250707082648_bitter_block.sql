/*
  ...
  1. New Tables
    ...
    - `quizzes` - Quiz definitions with metadata and settings
    ...
    - `user_answers` - Detailed user responses to questions

  2. Security
    - Enable RLS on all tables
    - Add comprehensive policies for user and admin access
    - Create trigger function for automatic profile creation

  3. Sample Data
    - Insert sample quizzes, questions, and options for testing
*/
...
INSERT INTO public.quizzes (id, title, description, category, difficulty, time_limit, created_by) 
SELECT * FROM (VALUES
...
) AS v(id, title, description, category, difficulty, time_limit, created_by)
WHERE NOT EXISTS (SELECT 1 FROM public.quizzes WHERE id = v.id);

-- Insert sample questions only if they don't exist
INSERT INTO public.questions (id, quiz_id, type, question, points, order_index)
SELECT * FROM (VALUES
...
) AS v(id, quiz_id, type, question, points, order_index)
...
) AS v(question_id, text, is_correct)
WHERE NOT EXISTS (
  SELECT 1 FROM public.question_options 
  WHERE question_id = v.question_id AND text = v.text
);